...
Pour chaque article ci-dessous, le système a DÉJÀ calculé une quantité à commander (systemSuggestedQuantity), selon sa formule habituelle : vente moyenne hebdomadaire × (1 + safetyStockRatio, le taux de marge de sécurité visé) ramené au délai de réapprovisionnement (receptionLeadTimeDays jours, si fourni, sinon 7 jours par défaut), moins le stock actuel et les commandes déjà en cours. Ton rôle n'est PAS de recalculer une quantité indépendante à partir de zéro, mais de VÉRIFIER si ce chiffre est cohérent avec les données détaillées ci-dessous, et de l'AJUSTER seulement si tu identifies une raison concrète de le faire :
...
- l'écart de saisonnalité (seasonalityDeviationPct, % par rapport à la même période l'an dernier si disponible, positif = hausse) : une hausse confirmée par la saisonnalité mérite plus de confiance qu'une simple variation récente dans dailyHistory
- la méthode de prévision utilisée (forecastMethod: "flat" = moyenne plate faute d'historique suffisant, "smoothed" = lissage exponentiel) : une prévision "flat" est moins fiable, une hausse/baisse y est plus facilement un artefact du peu de données disponibles
- le score de confiance du système dans sa propre prévision (systemConfidenceScore, 0-100 si disponible) : plus il est bas, moins la base de calcul est fiable, et plus un ajustement réfléchi de ta part a de valeur
